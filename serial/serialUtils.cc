...
int serialPortOpen(){
	...
	if (serial_port == -1) {
		perror("Error opening serial port, trying acm2");
		int serial_port = open("/dev/ttyACM0", O_RDWR);
...
int start(int serial_port_in){
	serialPortWrite("start", serial_port_in);
	return serialPortWrite("\n", serial_port_in);

}

int stop(int serial_port_in){
	int status = 1;
	status = serialPortWrite("stop", serial_port_in);
	status = serialPortWrite("\n", serial_port_in);
	return status;
}

int turn(int serial_port_in, char* degrees){
	int status = 1;
	status = serialPortWrite("steer:", serial_port_in);
	status = serialPortWrite(degrees , serial_port_in);
	status = serialPortWrite("\n" , serial_port_in);
	return status;
}
int set_speed(int serial_port_in, char* speed){
	int status = 1;
	status = serialPortWrite("speed:", serial_port_in);
	status = serialPortWrite(speed , serial_port_in);
	status = serialPortWrite("\n" , serial_port_in);
	return status;
}
